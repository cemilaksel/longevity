...
import { GPTType, useGPTStorage, formatChatDate } from '../hooks/useGPTStorage';
...
export const GPTButtons: React.FC<GPTButtonsProps> = ({ gptType, title, newChatUrl }) => {
  const { saveChatURL, getSession } = useGPTStorage();
  ...
  const handleSave = () => {
    const success = saveChatURL(gptType, urlInput);
    if (success) {
      setUrlInput('');
  ...
  return (
    <div className="mb-6 bg-gray-50 p-4 md:p-6 rounded-3xl border border-gray-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
        ...
        >
          {session ? `📝 Son Sohbet (${formatChatDate(session.date)})` : '📝 Son Sohbet (Yok)'}
        </button>
