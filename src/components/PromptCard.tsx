...
export const PromptCard: React.FC<PromptCardProps> = ({ prompt, copiedId, onCopy, activePersonName }) => {
  const processedContent = activePersonName 
    ? prompt.content.replace(/\[AKTIF_KISI\]/g, activePersonName === 'Ben' ? 'benim' : `${activePersonName} için`)
    : prompt.content;
  ...
  return (
    <div className={`p-6 rounded-3xl border border-gray-100 ${prompt.bgColor || 'bg-gray-50'} relative group overflow-hidden space-y-4`}>
      ...
      <div className="relative group">
        ...
        <button
          onClick={() => onCopy(processedContent, prompt.id)}
